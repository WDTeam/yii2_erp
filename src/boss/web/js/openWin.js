...
//        }else{
            ...
            var sWinBg = winBg.replace('http://cms/Apps/Admin/static/default/images/','');
...
//        }
    ...
    displayAlert:function(txt, winId){
        ...
        txtstr += '       <div class="txtBxo"><div class="alertwin"><em class="warring"></em><span style="line-height:72px;">'+txt.html+'</span></div></div>';
        txtstr += '       <div class="txtBtnBox"><input type="button" wId="'+winId+'" onclick="'+txt.yesFunc+'()" class="formcle" id="formcle" value="确定"/></div>';
        txtstr += '   </div>';
    ...
    displayConfirm:function(txt, winId){
        var txtstr = '<div class="txtWinBox">';
        txtstr += '       <div class="txtBxo"><div class="confirmwin"><em class="confirm"></em><span>'+txt.html+'</span></div></div>';
        txtstr += '       <div class="txtBtnBox btn-group"><input type="button" wId="'+winId+'" onclick="'+txt.yesFunc+'()" class="cancel btn btn-primary" id="openWin_yes" value="确定"/><input type="button" wId="'+winId+'" onclick="$.win.close($(this));return false;" class="formcle  btn btn-cancel" id="formcle" value="取消"/></div>';
        txtstr += '   </div>';
        $('#winBody_'+winId).html(txtstr);
        if(this.iscover){$('#cover_'+winId).show();}
        $('#openWin_'+winId).fadeIn('slow');
    ...
    displayTxt:function(txt,winId){
        var txtstr = '<div class="txtWinBox">';
        txtstr += '       <div class="txtBxo">'+txt.html+'</div>';
        txtstr += '       <div class="btn-group"><input type="button" wId="'+winId+'" onclick="'+txt.yesFunc+'()" class="cancel" id="formcle" value="确定"/></div>';
        txtstr += '   </div>';
        $('#winBody_'+winId).html(txtstr);
        if(this.iscover){$('#cover_'+winId).show();}
        $('#openWin_'+winId).fadeIn('slow');
    ...
    displayPic:function(url, winId){
        var html = '<img />';
        var obj = this;
        $(html).attr('src', url).load(function(){
            $('#winBody_'+winId).html($(this));
            if(obj.iscover){
                $('#cover_'+winId).show();
            }
            $('#openWin_'+winId).fadeIn('slow');
...
//        this.status[winId] = 1;
    ...
    getHtml:function(btns){
        ...
        for(var i = 0; i < btns.length; i++){
            if(btns[i] == 'closeWin'){var icon = 'fa-times'}
            ...
            str += '<i class="fa '+icon+' '+btns[i]+'" wId="'+winId+'" id="'+btns[i]+'_'+winId+'"></i>';
        }
        var html = '<div id="cover_'+winId+'" wId="'+winId+'" class="cover"></div>';/*onclick="$.win.close($(this));"*/
        html += '<div id="openWin_'+winId+'" class="openWin">';
        html += '   <h1 id="tWin_'+winId+'" class="tWin"><i class="fa fa-desktop ticon"></i><span class="titleWin" id="titleWin_'+winId+'"></span>'+str+'</h1>'
...
/*            obj.animate({'left':poses.left});*/
            ...
            var left = Math.round(($('body').outerWidth()- width)/2);
...
/*            obj.animate({'left':left});*/
    ...
    getSize:function(winId){
        var w = $('#openWin_'+winId).outerWidth();
        var h = $('#openWin_'+winId).outerHeight();
