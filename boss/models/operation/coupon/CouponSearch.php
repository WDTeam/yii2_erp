...
class CouponSearch extends Coupon
{
    ...
    public function search($params)
    {
		...
		if(isset($params['CouponSearch']['coupon_code'])){
			$coupon_code_arr = CouponCode::find()->where(['like', 'coupon_code', $params['CouponSearch']['coupon_code']])->asArray()->all();
			$coupon_ids = array();
			if(!empty($coupon_code_arr)){
				foreach($coupon_code_arr as $coupon_code){
					array_push($coupon_ids, $coupon_code['coupon_id']);
				}		
			}
