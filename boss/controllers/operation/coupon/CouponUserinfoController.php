	...
	public function actionCreate() {
		...
		if ($model->load ( Yii::$app->request->post () )) {
			...
			if ($dateinfo ['CouponUserinfo'] ['coupon_rule_name_id'] == '' && $dateinfo ['CouponUserinfo'] ['coupon_rule_name_id'] == 0) {
				...
				$coupon_rule = CouponRule::find ()->where ( [ 
						'id' => $dateinfo ['CouponUserinfo'] ['coupon_rule_name_id'] 
				] )->asArray ()->one ();
			...
			$ruledatainfo = \core\models\operation\coupon\CouponRule::find ()->select ( 'id,couponrule_name' )->where ( [ 
					'is_del' => 0,
					'is_disabled' => 0 
			] )->asArray ()->all ();
