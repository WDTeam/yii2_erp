    ...
    public static function pushToWorkers($order_id, $workers, $identity)
    {
        ...
        foreach ($workers as $v) {
            ...
            if (!in_array($v['id'], $is_jpush_worker_ids)) {
                ...
                $result = Yii::$app->jpush->push(["worker_{$v['worker_phone']}"],"阿姨，您有一个{$order->order_money}元的待抢订单，请及时确认接单。",json_encode(["order_code"=>$order->order_code,"msg"=>"阿姨，您有一个{$order->order_money}元的待抢订单，请及时确认接单。"]));
                if ($result) {
